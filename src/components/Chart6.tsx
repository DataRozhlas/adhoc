...
function prepareData(
  ...
  if (medalMethod === "all" && relativeMethod === "abs") {
    return medals.map((item: any) => ({
      cz: item.cz,
      t: -(item.g20 + item.s20 + item.b20 - item.g24 - item.s24 - item.b24),
    }));
  }
  if (medalMethod === "gold" && relativeMethod === "abs") {
    return medals
      .map((item: any) => ({ cz: item.cz, t: -(item.g20 - item.g24) }))
      .sort((a, b) => {
        const diff = b.t - a.t;
        if (diff === 0) {
  ...
  if (medalMethod === "all" && relativeMethod === "rel") {
    return medals
      .map((item: any) => ({
        cz: item.cz,
        t:
          (item.g24 + item.s24 + item.b24 - item.g20 - item.s20 - item.b20) /
          (item.g20 + item.s20 + item.b20),
      }))
      .sort((a, b) => {
        const diff = b.t - a.t;
        if (diff === 0) {
  ...
  if (medalMethod === "gold" && relativeMethod === "rel") {
    return medals
      .filter((item) => item.g20 > 0)
      .map((item: any) => ({
        cz: item.cz,
        t: (item.g24 - item.g20) / item.g20,
      }))
      .sort((a, b) => {
        const diff = b.t - a.t;
        if (diff === 0) {
      ...
      .map((item: any) => ({
        ...
        t:
          (4 * item.g24 +
            ...
            2 * item.s20 -
            item.b20) /
          (4 * item.g20 + 2 * item.s20 + item.b20),
      }))
      .sort((a, b) => {
        const diff = b.t - a.t;
        if (diff === 0) {
...
function Chart6() {
  ...
  useEffect(() => {
    const czechiaCountry = data.find((country: any) =>
      country.cz.toString().includes("Česká republika")
    );
    if (czechiaCountry) {
  ...
  return (
    <div className="max-w-[620px] mx-auto">
      <h1 className="text-2xl font-bold">
        Pořadatelský „doping“: srovnání LOH 2024 a 2020
      </h1>
      <p className="pb-4">
        Ve srovnání s předchozí olympiádou v Tokyu přidala nejvíc medailí
        pořadatelská Francie. Naopak největší propad zaznamenalo v absolutních
        číslech Japonsko. Česko je třetí od konce. Počítáme jen státy, které na
        obou akcích získaly aspoň jednu medaili.
      </p>
      ...
      <div className="flex flex-wrap pb-4">
        <div className="pb-4 md:w-1/2">
          ...
          <RadioGroup
            defaultValue="all"
            onValueChange={(value) => setMedalMethod(value)}
          >
        ...
        <div className="pb-4 md:w-1/2">
          <h2 className="font-bold pb-1">
            Výsledek v medailích, nebo v procentech?
          </h2>
          <RadioGroup
            defaultValue="abs"
            onValueChange={(value) => setRelativeMethod(value)}
          >
            <div className="flex items-center space-x-2">
      ...
      <div>
        <HighchartsProvider Highcharts={Highcharts}>
              ...
              series: {
                ...
                dataLabels: {
                  ...
                  formatter: function (this: any) {
                    if (
                      this.point.index === 0 ||
                      this.point.index === data.length - 1
                    ) {
                      ...
                      return `${this.point.y?.toLocaleString("cs", {
                        signDisplay: "exceptZero",
                        style: relativeMethod === "rel" ? "percent" : "decimal",
                      })} ${words()}`;
                    }
                    return this.point.y?.toLocaleString("cs", {
                      signDisplay: "exceptZero",
          ...
          >
            ...
            <XAxis
              categories={data.map((country) => String(country.cz))}
              labels={{
              ...
              plotLines={[
                {
                  ...
                  dashStyle: "ShortDot",
                  label: {
                    text: `Česko ${czechia.toLocaleString("cs", {
                      signDisplay: "exceptZero",
                      style: relativeMethod === "rel" ? "percent" : "decimal",
                    })}`,
                    rotation: 0,
            ...
            >
              <BarSeries
                name={
                  medalMethod === "gold"
                    ? "zlaté medaile"
                    : medalMethod === "weighted"
                    ? "vážené medaile"
                    : "medaile"
                }
                data={data.map((country) => Number(country.t))}
                color={"#5a6ff6"}
                negativeColor={"#f66060"}
              />
            </YAxis>
          </HighchartsChart>
          <p className="text-xs text-end">
            Zdroj dat:{" "}
            <a href="https://medalspercapita.com/">Olympic Medals per Capita</a>
          </p>
        </HighchartsProvider>
      </div>
    </div>
