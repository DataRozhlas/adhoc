  ...
  return (
    <HighchartsProvider Highcharts={Highcharts}>
      <h1 className="text-2xl font-bold">
        Jak se změnily průměrné platy na ministerstvech
      </h1>
      <h2>
        Ministerstvo obrany údaje za rok 2022 neposkytlo – jen sloučené s platy
        vojáků
      </h2>
      <HighchartsChart
        plotOptions={{
      ...
      >
        ...
        <XAxis categories={data.map((line) => String(line[0]))}></XAxis>

        <YAxis>
          <YAxis.Title>Průměrné platy (Kč)</YAxis.Title>
          <BarSeries
            name="2022"
            data={data.map((line) => Number(line[line.length - 3]))}
            color={colors[1]}
            datalbels={{ enabled: false }}
          />
          <BarSeries
            name="2024"
            data={data.map((line) => Number(line[line.length - 1]))}
            color={colors[0]}
            dataLabels={{
              enabled: true,
              inside: true,
              align: "right",
              formatter: function () {
                return `${this.y?.toLocaleString("cs-CZ") || null} Kč`;
              },
            }}
          />
        </YAxis>
      ...
      <p className="text-xs text-end">
        Údaje ze Státního závěrečného účtu (2022) a ze schváleného rozpočtu
        (2024)
      </p>
    </HighchartsProvider>
  );
